...
# Define product inventory with popularity
products = [
...
# Normalize popularity values to create a probability distribution
...
# Generate weighted list of product choices for restocking
product_choices = [p for p in products for _ in range(int(p["popularity"] * 1000))]

def generate_restock_event():
    product = random.choice(product_choices) 
    
    # Define a reasonable restock quantity based on popularity
    base_quantity = random.choices([10, 20, 50, 100, 200], weights=[50, 30, 10, 7, 3])[0]
    
    # Less popular items have lower restock sizes
    restock_quantity = max(5, int(base_quantity * product["popularity"] * 10))
    
    event = {
        "restock_time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%SZ"),
        "product_id": product["product_id"],
        "quantity_restocked": restock_quantity
    }
