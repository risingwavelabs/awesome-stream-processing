...
POLL_ATTEMPTS=0; MAX_POLL_ATTEMPTS=12; COLUMNS_COUNT=0
until [[ $COLUMNS_COUNT -gt 0 || $POLL_ATTEMPTS -ge $MAX_POLL_ATTEMPTS ]]; do
    ...
    COLUMNS_COUNT=$(echo "$DATASET_DETAILS" | jq '.result.columns | length'); if [[ $COLUMNS_COUNT -gt 0 ]]; then echo " Found $COLUMNS_COUNT columns."; else echo " No columns yet. Retrying..."; sleep 5; fi
done
if [[ $COLUMNS_COUNT -eq 0 ]]; then echo "❌ ERROR: Dataset columns not found." >&2; exit 1; fi
...
if ! echo "$UPDATE_DATASET_RESPONSE" | jq -e '.result.main_dttm_col=="timestamp"' > /dev/null; then echo "❌ Failed to set datetime column." >&2; exit 1; fi
...
for metric_name in $(echo "$DESIRED_METRICS" | jq -r 'keys[]'); do
    ...
    if [[ "$metric_exists" == "false" ]]; then
        ...
        expression=$(echo "$DESIRED_METRICS" | jq -r ".${metric_name}"); verbose_name=$(echo "$metric_name"|tr '_' ' '|awk '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) substr($i,2)} 1')
        ...
        if echo "$UPDATE_RESPONSE"|jq -e '.result'>/dev/null; then echo "    ✅ Successfully added metric '$metric_name'." >&2; else echo "    ❌ Failed to add metric '$metric_name'." >&2; exit 1; fi
    else echo "    - Metric already exists. Skipping." >&2; fi
done
...
POSITION_JSON=$(jq -n --argjson c1_id "$CHART_1_ID" '{ "DASHBOARD_VERSION_KEY":"v2","ROOT_ID":{"type":"ROOT","id":"ROOT_ID","children":["GRID_ID"]},"GRID_ID":{"type":"GRID","id":"GRID_ID","children":["ROW_ID_1"],"meta":{}},"ROW_ID_1":{"type":"ROW","id":"ROW_ID_1","children":["CHART_ID_1"],"meta":{"background":"BACKGROUND_TRANSPARENT"}},"CHART_ID_1":{"type":"CHART","id":"CHART_ID_1","children":[],"meta":{"width":12,"height":50,"chartId":$c1_id,"uuid":"chart-uuid-1"}}}'|jq -c .|jq -Rs .)
