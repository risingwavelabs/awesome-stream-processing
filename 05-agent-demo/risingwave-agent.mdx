...
The MessageHistory class manages the conversation between the user and the agent. It keeps a list of messages, tracks the number of tokens used to stay within the model’s context window limit, and allows messages to be truncated when necessary. It also tracks token usage for each message exchange, formats messages in a structure suitable for Anthropic’s API, and optionally adds cache control headers to recent messages to optimize performance.
...
The create_mcp_connection function serves as a factory to create either a stdio or sse MCP connection based on the configuration provided. The setup_mcp_connections async function loops through a list of server configs, creates connections, retrieves each server’s available tools, and dynamically generates async wrapper functions that the agent can later call. It collects both the tool definitions and their corresponding function wrappers into dictionaries for use in the agent loop.
...
This simple dataclass called ModelConfig defines the agent’s configurable model parameters. It sets defaults for which Claude model to use, how many tokens a response can contain, the model’s temperature (which controls response randomness), and the maximum number of tokens allowed in the conversation history window.
...
The Agent class is the core of this implementation. The constructor (__init__) initializes the agent’s name, system prompt, optional MCP servers and configuration, logging verbosity, the Anthropic API client, and message parameters. It also creates a new MessageHistory instance to track conversation state and token usage for each interaction.
...
The _prepare_message_params method builds a dictionary of parameters to send with each call to Anthropic’s messages.create() API. This includes the model config, system prompt, formatted conversation history, any available tools, and any additional custom message parameters provided during initialization.
...
The _agent_loop method is the heart of the agent’s functionality. It begins by adding the user’s message to the history, then continuously calls the Anthropic API to get the agent’s next response. If the response includes tool calls (commands for the agent to run a tool hosted on an MCP server), it executes them asynchronously using the previously generated wrappers, appends the tool results to the history, and continues the loop. The loop only exits when the agent’s response no longer includes any tool calls, at which point it returns the final response.
...
Finally, the run_async method is the agent’s top-level asynchronous entry point. It opens all MCP server connections, retrieves tool definitions, and runs the _agent_loop with the given user input. The run method wraps this with asyncio.run so it can be used in synchronous contexts — useful for scripts, CLIs, or other programs without native async support.
...
This file will work as the actual DNA of the agent, where you will identify its purpose and also give the proper configurations to ensure full functionality. Feel free to edit it to specific needs.
...
2. Show me the uders table structure
...
When you are done you can have the agent drop the users table or just close the terminal.
