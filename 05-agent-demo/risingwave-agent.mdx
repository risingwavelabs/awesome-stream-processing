...
Viewing data in RisingWave can be difficult and disorganized at times. A powerful tool to level the playing field is to implement an AI Agent to receive quick and accurate information about data.

Through Model Context Protocol's (MCP's) You can now easily connect any AI Agent and analyzing some data using natural language only.
...
* Ensure you have an [Anthropic](https://console.anthropic.com/settings/keys) API key. Do not worry about tokens, this demo shouldn't spend more than -----CHECK BEFORE PUBLISHING-----.
